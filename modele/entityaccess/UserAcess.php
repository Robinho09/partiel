...
class UserAcess implements modele
{
    ...
    {
        $stm = $this->connection->query("select * from users ");
        $users = $stm->fetchAll();
        return $users;
    }
    ...
    {
        $stm = $this->connection->prepare("select * from users where ( username like ? or email like ? ) and (id != ?) ");
    ...
    {
        $stm = $this->connection->prepare("select * from users where username = ? and password = ? ");
        $stm->bindValue(1,$username);
    ...
    {
        $stm = $this->connection->prepare("update user set  email = ? , username = ? , date_inscription = ? , where id = ?");
